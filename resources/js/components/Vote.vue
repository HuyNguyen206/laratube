<template>
    <div class="d-flex">
        <div class="mr-3">
            <svg @click="vote()" style="width: 25px" aria-hidden="true" focusable="false" data-prefix="far"
                 data-icon="thumbs-up"
                 class="svg-inline--fa fa-thumbs-up fa-w-16 thumb-up" :class="{'thumb-up-active': isUpVote}" role="img"
        ...
        <div>
            <svg @click="vote('down')" style="width: 25px" aria-hidden="true" focusable="false" data-prefix="far"
                 data-icon="thumbs-down"
                 class="svg-inline--fa fa-thumbs-down fa-w-16 thumb-down" :class="{'thumb-down-active': isDownVote}"
                 role="img"
                 xmlns="http://www.w3.org/2000/svg"
                 viewBox="0 0 512 512">
                <path fill="currentColor"
    ...
    computed: {
        upVotes() {
            return this.votes.filter((vote) => {
                return vote.pivot.type === 'up';
            })
        },

        downVotes() {
            return this.votes.filter((vote) => {
                return vote.pivot.type === 'down';
            })
        },
        upVoteCount() {
        ...
        isUpVote() {
            return !!this.upVotes.find(vote => {
                return authUser && vote.id === authUser.id
            })
        },
        isDownVote() {
            return !!this.downVotes.find(vote => {
                return authUser && vote.id === authUser.id
            })
        }
    }
